layui.define(['jquery', 'util', 'element', 'rate', 'proxy'], function(exports){
            ...
            render_: function(container){
                ...
                if (content == undefined) {
                    ...
                    <div class="layui-footer" style="margin:2em;">                                                                                              \
                        <!-- 底部固定区域 -->                                                                                                \
                        <p style="display:flex; justify-content: center;">© 2020-2024 iTechX - MIT license.</p>                             \
                    </div>';
                    return;
                }
                ...
                const gitalk = new Gitalk({
                    ...
                    baseURL: proxy.parse(options.baseURL)
                })
                ...
                renderRating = function (r, selfRating, issueID) {
                    ...
                    layui.rate.render({
                        ...
                        ,choose: function(value){
                            ...
                            if (selfRating) {
                                $.ajax({
                                    url: proxy.parse(options.baseURL + "/repos/" + options.owner + "/" + options.repo + "/issues/" + issueID + "/reactions/" + selfRating.id),
                                    type: "DELETE",
                                    headers: headers,
                                    success: function (data) {
                                        $.ajax({
                                            url: proxy.parse(options.baseURL + "/repos/" + options.owner + "/" + options.repo + "/issues/" + issueID + "/reactions"),
                                            type: "POST",
                                            data: JSON.stringify({
                                                'content': value
                                            }),
